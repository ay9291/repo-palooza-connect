...
import AdminAlertsPanel from "@/features/premium/components/AdminAlertsPanel";
import BulkPricingPanel from "@/features/premium/components/BulkPricingPanel";
import TopProductsPanel from "@/features/premium/components/TopProductsPanel";
import { buildAdminKpis, buildPartnerAssignmentStats, filterOrdersByStatus } from "@/features/premium/modules/admin-insights";
import { applyBulkDiscount, buildOperationalAlerts, buildOrdersCsv, filterProducts, summarizeTopProducts } from "@/features/premium/modules/admin-advanced";
...
const AdminDashboardPage = () => {
  ...
  const loadOps = async () => {
    ...
    const { data: a } = await supabase
      ...
      .select("id, partner_id, delivery_status, created_at")
      .order("created_at", { ascending: false })
      .limit(150);
    const { data: oi } = await supabase
      .from("premium_order_items")
      .select("quantity, unit_price, product:premium_products(name)")
      .limit(300);

    setOrders((o || []) as OrderRow[]);
    setPartners((p || []) as PartnerRow[]);
    setAssignments((a || []) as AssignmentRow[]);
    const normalizedItems = ((oi || []) as Array<{ quantity: number; unit_price: number; product: { name: string } | null }>).map((row) => ({
  ...
  const restockAllLowStock = async () => {
    ...
    await Promise.all(
      lowStock.map((product) => supabase.from("premium_products").update({ stock_quantity: product.stock_quantity + 20 }).eq("id", product.id)),
    );
    await refreshProducts();
  };

  const applyDiscountToAllActive = async () => {
    const targets = applyBulkDiscount(products.filter((product) => product.is_active), discountPercent);
    await Promise.all(
      targets.map((target) => supabase.from("premium_products").update({ price: target.nextPrice }).eq("id", target.id)),
    );
    await refreshProducts();
  };
  ...
  const exportOrdersCsv = () => {
    ...
    link.href = url;
    link.setAttribute("download", "premium-orders-export.csv");
  ...
  const statusFilteredOrders = useMemo(() => filterOrdersByStatus(orders, statusFilter), [orders, statusFilter]);
  const filteredOrders = useMemo(() => {
    return statusFilteredOrders.filter((order) => {
      const timestamp = new Date(order.created_at).getTime();
      const afterFrom = fromDate ? timestamp >= new Date(`${fromDate}T00:00:00`).getTime() : true;
      const beforeTo = toDate ? timestamp <= new Date(`${toDate}T23:59:59`).getTime() : true;
      return afterFrom && beforeTo;
    });
  }, [statusFilteredOrders, fromDate, toDate]);

  const partnerStats = useMemo(() => buildPartnerAssignmentStats(partners, assignments), [partners, assignments]);
  const productCategories = useMemo(() => [...new Set(products.map((product) => product.category || "uncategorized"))], [products]);
  const visibleProducts = useMemo(() => filterProducts(products, productQuery, categoryFilter), [products, productQuery, categoryFilter]);
  const alerts = useMemo(() => buildOperationalAlerts(orders, assignments), [orders, assignments]);
  const topProducts = useMemo(() => summarizeTopProducts(orderItems), [orderItems]);
  ...
  return (
    <PremiumShell>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Button variant="outline" onClick={() => { refreshProducts(); loadOps(); }}>Manual Refresh</Button>
      </div>
      ...
      <div className="grid xl:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          <BulkPricingPanel
            ...
            onApplyDiscount={applyDiscountToAllActive}
            onRestockAllLowStock={restockAllLowStock}
          />
          ...
          <div className="space-y-2">
            {visibleProducts.map((product: PremiumProduct) => (
        ...
        <div className="space-y-3">
          ...
          <AdminAlertsPanel
            stalePlacedOrders={alerts.stalePlacedOrders}
            longRunningAssignments={alerts.longRunningAssignments}
          />

          <h2 className="text-lg font-semibold mt-4">Low Stock Alerts</h2>
          <LowStockManager products={products} onRestock={restockProduct} />

          <h2 className="text-lg font-semibold mt-4">Delivery Partner Performance</h2>
          <PartnerPerformancePanel stats={partnerStats} />

          <h2 className="text-lg font-semibold mt-4">Top Products by Revenue</h2>
